...
type ExampleGenerator = (url: string, data: ExampleRequestData, context?: unknown) => string;
...
const jsFetchSample: ExampleGenerator = (url, data) => {
  const lines: string[] = [];
...
const phpSample: ExampleGenerator = (url, data) => {
...
export function generateCobruCodeSamples(): CodeSample[] {
  const generators: Record<GeneratorKey, ExampleGenerator> = {
    fetch: jsFetchSample,
